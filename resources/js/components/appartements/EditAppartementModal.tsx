...
import { Immeuble, Appartement, Client } from "../../types";

interface EditAppartementModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const EditAppartementModal: React.FC<EditAppartementModalProps> = ({
    isOpen,
    onClose,
    onSave,
    appartement,
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            axios.get("/api/clients?paginate=false").then((res) => {
                const data = Array.isArray(res.data)
                    ? res.data
                    : res.data.data || [];
                setClients(data);
            ...
            if (appartement) {
                setFormData({
                    ...
                    immeuble_id: appartement.immeuble_id || "",
                    etage: appartement.etage?.toString() || "",
                    surface: appartement.surface?.toString() || "",
                    chambres: appartement.chambres?.toString() || "",
                    prix_total: appartement.prix_total?.toString() || "",
                    statut: appartement.statut || "disponible",
                    description: appartement.description || "",
                    client_id: appartement.client_id || "",
                });
            }
        }
    }, [isOpen, appartement]);

    if (!isOpen || !appartement) return null;
...
export default EditAppartementModal;
